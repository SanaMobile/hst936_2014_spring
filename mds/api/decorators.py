...
def validate(operation='POST'):
    ...
    def wrap(f, handler, request, *a, **kwa):
        ...
        if operation == 'POST':
            # TODO Probably safe to remove this
            #data = getattr(request, operation)
            #form = v_form(data=data, request.FILES) if request.FILES else v_form(data=data)
            form = v_form(request.POST,request.FILES) if request.FILES else v_form(request.POST)
            if not form.is_valid():
                #errs = list((key, [unicode(v) for v in values]) for key,values in form.errors.items())
                errs = form.errors.keys();
                #errs = [unicode(v) for v in in form.errors.items()]
